...
Public Const gmsgUpdateAskActiveContentMod As String = "アクティブプレゼンテーションは変更されています。更新を中止しますか？「いいえ」を選択すると変更は破棄され、更新を継続して行います。"
Public Const gmsgCommitErrActiveContentFileReadOnly As String = "コミットできません。アクティブプレゼンテーションは変更されていますが、ファイル属性が読み取り専用となっています。"
Public Const gmsgCommitAskSaveMod As String = "アクティブプレゼンテーションには変更があります。上書き保存しますか？"
Public Const gmsgCommitAskSaveModCloseReopen As String = "コミット時に、ファイルをいったん閉じて再度開きます。アクティブプレゼンテーションには変更があります。上書き保存しますか？"
Public Const gmsgLockAskActiveContentFileReadOnly As String = "アクティブプレゼンテーションは変更されていますが、ファイル属性が読み取り専用となっています。ロック取得を中止しますか？「いいえ」を選択すると変更は破棄され、ロック取得を継続して行います。"
Public Const gmsgLockAskSaveModContent As String = "ロックを取得時に、ファイルをいったん閉じて再度開きます。アクティブプレゼンテーションには変更があります。上書き保存しますか？"
Public Const gmsgUnlockErrActiveContentFileReadOnly As String = "ロックを開放できません。アクティブプレゼンテーションは変更されていますが、ファイル属性が読み取り専用となっています。"
Public Const gmsgUnlockAskActiveContentMod As String = "アクティブプレゼンテーションは変更されています。ロックの開放では変更内容をリポジトリへ反映することはできません。続行しますか?"
Public Const gmsgAddAskCommit As String = "続けてコミットを実行しますか?"
Public Const gmsgErrNotSaveFile As String = "ファイルを保存することはできませんでした。"
Public Const gmsgErrActiveContentFileNotExist As String = "アクティブプレゼンテーションのファイルがありません。プレゼンテーションをファイルに保存してからこの操作を行ってください。"
Public Const gmsgErrFolderNotUnderCtrl As String = "アクティブプレゼンテーションはバージョンコントロール下のフォルダにありません。"
Public Const gmsgErrFileNotUnderCtrl As String = "アクティブプレゼンテーションはバージョンコントロールされていません。"
