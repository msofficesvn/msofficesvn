...
Public Const gmsgUpdateAskActiveContentMod As String = "The active book is modified. Do you abort updating? If you select [No], the changes are discarded and continue updating."
...
Public Const gmsgCommitAskSaveModContent As String = "Once the active book file is closed, and reopened after committing. The active book is modified. Do you save it?"
Public Const gmsgLockAskActiveContentFileReadOnly As String = "The active book is modified but the file attribute is read only. Do you wish to abort getting lock? If you select [No], the changes are discarded and continue getting lock."
...
Public Const gmsgUnlockErrActiveContentFileReadOnly As String = "Can't release lock of the active book. It is modified but the file attribute is read only."
Public Const gmsgUnlockAskActiveContentMod As String = "The active book is modified. You can't put the changes into the repository by realeasing lock. Do you continue?アクティブブックは変更されています。ロックの開放では変更内容をリポジトリへ反映することはできません。続行しますか?"
Public Const gmsgAddAskCommit As String = "Do you go on to commit it?続けてコミットを実行しますか?"
Public Const gmsgErrNotSaveFile As String = "Failed to save the file.ファイルを保存することはできませんでした。"
Public Const gmsgErrActiveContentFileNotExist As String = "Can't find the file of the active book. Save the active book and retry. アクティブブックのファイルがありません。ブックをファイルに保存してからこの操作を行ってください。"
Public Const gmsgErrFolderNotUnderCtrl As String = "The file of the active book is not in the folder under version control.アクティブブックはバージョンコントロール下のフォルダにありません。"
Public Const gmsgErrFileNotUnderCtrl As String = "The file of the active book is not under version control.アクティブブックはバージョンコントロールされていません。"
